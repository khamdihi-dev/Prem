            ...
            for self.ceks in open(_file,'r').read().splitlines():
                try:
                    ...
                    if 'sessionid' in self.ceks:
                        self.cokie = re.search('mid=(.*)',self.ceks).group(1)
                        self.sign = self.chekcokie(f'mid={self.cokie};')
                        if self.sign == True:
                            print('\nsuccess :',self.cokie)
                            exit()
            exit()
...
class menu:
    ...
    def instagram(self):
        ...
        elif self.sdyh == self.dihi[9]:exit('\nKamu harus menginstall (Berbayar/berbeda license)\ngit clone https://github.com/khamdihi-dev/2fa')
        elif self.sdyh == self.dihi[10]:exit('\nGood Bye')
    ...
    def findinboxkitten(self):
        ...
        if self.finterakun == 'y':
            ...
            self.medsos = questionary.select(self.ulsd.MedsosList()['head'],choices=self.listofmedsos).ask()
    ...
    def decode_bearer(self):
        try:
            print('\nTempel Bearer contoh : Bearer IGT:2:eyJkc191c2VyX2lkIjoiNDU0OTk2Njk2OTEiLCJzZXNzaW9uaWQiOiI0NTQ5OTY2OTY5MSUzQTVpRUxtYVhROTZCSW9jJTNBMTUlM0FBWWM5RjhyM1BXUHdfdVFpV0RqbGZCZTlZSGhpUVctUEQtYU1UVjg2NWcifQ==')
            code = input('Bearer token : ').split(':')[2]
...
instagram cookies   : {self.info['instagram cookies']}
    ...
    def followers(self):
        ...
        Brute.MAIN(bahasa[0]).List(dumpdata)
