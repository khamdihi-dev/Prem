...
<_____/\_____/\_____/|_____/  %ssimpel bruteforce %sv1.0%s
       '''%(P,H,P,P,H,P))
   def aset(self):
...
{P}[{H}U{P}] Update Script     [{H}9{P}] Log, Out\n''')
       self.chs(self.aset)
   ...
   def chs(self, assets):
       while True:
         ...
         elif x in ['u','U' ]:exit(os.system(f'git pull;python {sys.argv[0]}'))
       ...
       for i in auid:
          try:
              ...
              for xxx in url['data']:
                  ...
                  print(f'{P}[{H}+{P}] Berhasil dump {H}{len(self.id)}',end='\r');sys.stdout.flush()
...
# OS BUAT KALIAN PEJUANG SC FREE :)
